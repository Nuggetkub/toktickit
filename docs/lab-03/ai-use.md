...
| 10 | "continue our work" | Issue #52 — Public Comments, Internal Notes and the resolution indication. Two results I did not expect. First, **checking a contract row instead of assuming it found a defect in already-merged code**: API-21 has always promised that a closed ticket refuses an attachment upload and removal, and the attachment routes had no status check at all, so BR-27 was simply not enforced there. The row had been describing behaviour that did not exist since the contract was written. Second, and more useful to me: **the break-the-code run reported one of my own guards as unprovable.** Deleting the conditional `WHERE` that protects the resolution indication left every test green, because I had written the same rule twice and the outer `if` caught every case the tests could reach. The agent's instinct was to write a cleverer test; I took the other option — delete the duplicate rule, so one guard remains and a test can actually fail when it breaks. A guard no test can fail is indistinguishable from no guard at all. |
